...
# --- CREDENCIAIS ABSOLUTAS FIXAS (Ignora o painel de Secrets travado do Streamlit) ---
...
# --- FUNÇÕES DE COMUNICAÇÃO ISOLADA HTTP (Sem vazamento de variáveis) ---
...
if not st.session_state["user_logged"]:
    ...
    if st.session_state["user_type"] == "admin":
        ...
        # Proteção de desempacotamento para listas e dicionários no cabeçalho
        if isinstance(dados_cliente_topo, list) and len(dados_cliente_topo) > 0:
            cliente_info_topo = dados_cliente_topo[0]
        else:
            cliente_info_topo = dados_cliente_topo if dados_cliente_topo else {}
            
        nome_exibir = cliente_info_topo.get('nome_completo', 'Usuário')
...
if menu == "Área Administrativa" and not st.session_state["user_logged"]:
    ...
    if st.button("Acessar"):
        # Compara direto com o segredo estático do painel de administração
    ...
    with aba_login:
        ...
        if st.button("Entrar"):
            ...
            if dados_cli and not isinstance(dados_cli, dict) and len(dados_cli) > 0:
                ...
                st.session_state["cliente_dados"] = dados_cli
...
# --- TELAS DO SISTEMA: 3. PAINEL DO CLIENTE LOGADO ---
    ...
    if isinstance(dados_cli_busca, list) and len(dados_cli_busca) > 0:
        cliente_info_busca = dados_cli_busca[0]
    else:
        cliente_info_busca = dados_cli_busca if dados_cli_busca else {}
    ...
    if isinstance(dados_cli_perfil, list) and len(dados_cli_perfil) > 0:
        cliente_info_perfil = dados_cli_perfil[0]
    else:
        cliente_info_perfil = dados_cli_perfil if dados_cli_perfil else {}
